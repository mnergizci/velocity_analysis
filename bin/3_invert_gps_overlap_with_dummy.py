...
# from decomposition_with_uncertainties import *
...
def load_dummy_GNSS():
    crs = "EPSG:4326"
    polygon = gpd.GeoDataFrame(crs="EPSG:4326", geometry=[Point()])
    index = 0
    polygon_file = '/work/scratch-pw2/eeyga/Kokoxili_earthquake_postseismic_InSAR/polygon.txt'
    # Load the polygon coordinates from the specified TXT file
    fl = open(polygon_file, "r").readlines()
    for line in fl:
        column_1, lon, lat, s_los = line.split()
        polygon.loc[index, 'geometry'] = Point(float(lon), float(lat))
        index += 1
    return polygon
    # with open(polygon_file, "r") as f:
    #     coordinates = [line.strip().split() for line in f]
    #     polygon = Polygon([(float(lon), float(lat)) for lon, lat in coordinates])

    gps_gdf = gpd.GeoDataFrame(crs="EPSG:4326", geometry=[Point()])
    gps_gdf['geometry'] = None
    index = 0
    dummy = '/work/scratch-pw2/eeyga/Kokoxili_earthquake_postseismic_InSAR/plot_GNSS_vel/GNSS_interpolated_reformat.txt'

    fl = open(dummy, "r").readlines()
    for line in fl:
        lon, lat, Ve_E, Vn_E, dVe, dVn, Cen = line.split()
        ...
        gps_gdf.loc[index, 'su'] = 1  # sigma vu in ITRF20082008 reference frame
        index += 1
    # The ~ operator before gps_gdf.geometry.intersects(polygon) negates the condition, effectively keeping the rows where the geometries do not intersect with the polygon.
    # gps_gdf = gps_gdf[~gps_gdf.geometry.intersects(polygon)]
    gps_gdf = gps_gdf[gps_gdf.geometry.intersects(polygon)]
    return gps_gdf
...
def load_chris():
    ...
    gps_gdf['geometry'] = None
    index = 0
    gps2d_file = "/work/scratch-pw2/eeyga/Kokoxili_earthquake_postseismic_InSAR/earjcr1-AHB_GPS-1814ff5/tibet/eastahb_tol1.5_minocc2.5_dist5.dat"
...
def load_wang_GPS_2020():

    """ Load GPS data from Wang & Shen 2020 """
    gps_gdf = gpd.GeoDataFrame(crs="EPSG:4326")
    gps_gdf['geometry'] = None
    index = 0
    gps_file = "/work/scratch-pw2/eeyga/Kokoxili_earthquake_postseismic_InSAR/wang_2020.txt"
    fl = open(gps_file, "r").readlines()
    for line in fl:
        if not line.startswith(('*', "Table")):
            ...
            gps_gdf.loc[index, 've'] = float(Ve_E)  # eastern velocity in mm/yr in fixed eurasia reference frame
            gps_gdf.loc[index, 'vn'] = float(Vn_E)  # northern velocity in mm/yr in fixed eurasia reference frame
            gps_gdf.loc[index, 'vu'] = 0  # assuming vn = 0
            gps_gdf.loc[index, 'se'] = float(dVe)  # sigma ve
            gps_gdf.loc[index, 'sn'] = float(dVn)  # sigma vn
            gps_gdf.loc[index, 'cen'] = float(Cen)  # correlation between east and northern velocities
            gps_gdf.loc[index, 'su'] = 2  # sigma vu
            index += 1
    return gps_gdf

def load_wang_gps_2020_qicode():
    # gps_gdf = gpd.GeoDataFrame(crs="EPSG:4326")
    gps_gdf = gpd.GeoDataFrame(crs="EPSG:4326", geometry=[Point()])
    # gps_gdf = gpd.GeoDataFrame(crs="EPSG:4326", geometry=[Point() for _ in range(2402)])
    gps_gdf['geometry'] = None
    index = 0
    wang = '/work/scratch-pw2/eeyga/Kokoxili_earthquake_postseismic_InSAR/wang_2020.txt'
    ...
    for line in fl:
        if not line.startswith(('*', "Table")):
            ...
            #    gps_gdf.loc[index, 've'] = float(Ve_I)  # eastern velocity in mm/yr in ITRF2008 reference frame
            #    gps_gdf.loc[index, 'vn'] = float(Vn_I)  # northern velocity in mm/yr in ITRF2008 reference frame
            #else:
            ...
            # gps_gdf.loc[index, 'cen'] = float(Cen)  # correlation between east and northern velocities
            gps_gdf.loc[index, 'vu'] = float(0)  # up velocity in mm/yr in ITRF20082008 reference frame
            gps_gdf.loc[index, 'su'] = 2  # sigma vu in ITRF20082008 reference frame
            index += 1
    return gps_gdf

def load_zhao_3d_gps_2023():
    # gps_gdf = gpd.GeoDataFrame(crs="EPSG:4326")
    gps_gdf = gpd.GeoDataFrame(crs="EPSG:4326", geometry=[Point()])
    # gps_gdf = gpd.GeoDataFrame(crs="EPSG:4326", geometry=[Point() for _ in range(2402)])
    gps_gdf['geometry'] = None
    index = 0
    zhao = '/work/scratch-pw2/eeyga/interseismic_insar_gps/zhao_epsl_2023_3dGPS.txt'
    fl = open(zhao, "r").readlines()
    for line in fl:
        if not line.startswith(('*', "Table")):
            lon, lat, Ve, Vn, Vu, dVe, dVn, dVu, sta = line.split()
            gps_gdf.loc[index, 'geometry'] = Point(float(lon), float(lat))
            gps_gdf.loc[index, 'station'] = sta[:4]
            gps_gdf.loc[index, 've'] = float(Ve)  # eastern velocity in mm/yr in fixed eurasia reference frame
            gps_gdf.loc[index, 'vn'] = float(Vn)  # northern velocity in mm/yr in fixed eurasia reference frame
            gps_gdf.loc[index, 'se'] = float(dVe)  # sigma ve
            gps_gdf.loc[index, 'sn'] = float(dVn)  # sigma vn
            gps_gdf.loc[index, 'vu'] = float(Vu)  # up velocity in mm/yr in ITRF20082008 reference frame
            gps_gdf.loc[index, 'su'] = float(dVu)  # sigma vu in ITRF20082008 reference frame
            index += 1
    return gps_gdf
...
if __name__ == "__main__":
    ...
    maindir=os.getcwd()
    input_dir = 'ranges_vm4'  # where input velocity frames are stored
    input_suffix = '*.vel.no_inter.tif' #'*.vel_filt.mskd.geo.tif'
    sigma_dir = input_dir
    sigma_suffix = '*.vstd.geo_scaled.tif'
    NEU_dir = input_dir
    N_suffix = 'N.tif'
    E_suffix = 'E.tif'
    U_suffix = 'U.tif'
    gps_add = False
    # output_dir = '/work/scratch-pw2/eeyga/interseismic_insar_gps/reference_by_gps_overlap_weighted_v_vstd/'
    output_dir = 'reference_by_gps_overlap_dummy_gps'
    output_suffix = '_weighted'  # _jointly or #_weighted
    ...
    if gps_add:
        ...
        wang_gps = load_wang_gps_2020_qicode()
        zhao_gps = load_zhao_3d_gps_2023()
        chris_gps = load_chris()
        dummy_gps = load_dummy_GNSS()
        print("dummy_gps", dummy_gps)
        # all_gps = pd.concat([zhao_gps], ignore_index=True)
        all_gps = pd.concat([chris_gps, dummy_gps], ignore_index=True)
    ...
    tifList =  sorted(glob.glob(os.path.join(maindir, input_dir,'*' , input_suffix)))
    
    
    # 2. define tracks from frame names
    for tif_file in tifList:
        frame = os.path.splitext(os.path.basename(tif_file))[0][:17]
        track=frame[:4]
        print(frame, track) 
        # load insar los maps
        frameList = sorted(glob.glob(os.path.join(input_dir, track + '*', track+input_suffix)))
        sigfileList = sorted(glob.glob(os.path.join(input_dir, track + '*', track+sigma_suffix)))
        n_coefList=sorted(glob.glob(os.path.join(NEU_dir, track + '*', N_suffix)))
        e_coefList=sorted(glob.glob(os.path.join(NEU_dir, track + '*', E_suffix)))
        u_coefList=sorted(glob.glob(os.path.join(NEU_dir, track + '*', U_suffix)))
        ...
        for index, frame in enumerate(frameList):
            ...
            ntif=OpenTif(n_coefList[index])
            etif=OpenTif(e_coefList[index])
            utif=OpenTif(u_coefList[index])
            ds.append(tif)
            sigma.append(sigtif)
            n_coef.append(ntif)
            e_coef.append(etif)
        ...
        if gps_add:
            for index, tif in enumerate(ds):
                ...
                # calculate gps los
                add_NEU_los(gps, n_coef[index], e_coef[index], u_coef[index])
                ...
                gps['gps_sigma'] = np.sqrt(np.square(gps['insar_sigma'].to_numpy()) + np.square(gps['los_sigma'].to_numpy()))
                gps.dropna(inplace=True)
                # gps = gps[gps['insar']>-5] # to avoid using gps data in rapidly subsiding fields for referencing
                # gps = gps[gps['insar']<5] # to avoid using gps data in rapidly uplifting regions due to earthquakes
                ...
                if len(gps_3D) < frame_3d_gps_threshold:
                    ...
                    gps = gps_3D
                # breakpoint()
                # insar = [ds[index].extract_pixel_value(point.x, point.y, 10)[0] for point in gps['geometry']]
                # insar_sigma = [ds[index].extract_pixel_value(point.x, point.y, 10)[1] for point in gps['geometry']]
                # gps_sig = np.sqrt(np.square(insar_sigma) + np.square(gps['los_sigma'].to_numpy()))
        ...
        if len(frameList) > 1:
            ...
            for i in range(len(frameList)-1):
                ...
                non_nan_sig = sigover.vector_sum_array[~np.isnan(over.diff_array)]  # extract the same pixels from sigma overlap
            ...
            if gps_add:
                ...
                print(insar_offset_number)
                weight = 1
                all_offset = all_overlap_offset_with_sig
                all_g = all_overlap_g_with_sig
        ...
        for i in np.arange(len(frameList)):
            ...
            export_tif(ds[i].data_projected, ds[i], os.path.join(output_dir, ds[i].basename[:17] + output_suffix + '.tif'))
        ...
        # vmin = -2
        # vmax = 2
        im = axs[0].imshow(track_map, cmap=cm.roma.reversed(), vmin=-10, vmax=10, interpolation='nearest')
        axs[0].set_title("InSAR LOS", pad=2)
        cbar = ax.cax.colorbar(im, ticklocation='left')
        cbar.set_label("LOS velocity / mm per yr")
        ...
        axs[1].scatter(all_x, all_y, c=all_gps_offset, cmap=cm.roma.reversed(), vmin=-10, vmax=10)
        axs[1].set_title("GNSS Offset", pad=2)

        # im10 = axs[1, 0].imshow(track_map_proj, cmap=cm.roma, vmin=vmin, vmax=vmax)
        # axs[3].imshow(track_map_proj, cmap=cm.roma.reversed(), vmin=vmin, vmax=vmax)
        axs[3].imshow(ramps_map, cmap=cm.roma.reversed(), vmin=-10, vmax=10, interpolation='nearest')
        axs[3].set_title("Inverted Ramps", pad=2)

        axs[4].imshow(gps_map)  # empty map to define the dimension
        axs[4].scatter(all_x, all_y, c=all_gps_res, cmap=cm.roma.reversed(), vmin=-10, vmax=10)
        ...
        if len(frameList) > 1:
            ...
            axs[2].imshow(overlap_map, cmap=cm.roma.reversed(), vmin=-10, vmax=10, interpolation='nearest')
            ...
            axs[5].imshow(overlap_res_map, cmap=cm.roma.reversed(), vmin=-10, vmax=10, interpolation='nearest')
        ...
        # plot histograms
        number_of_hists = 2 * len(frameList)  # for each track, there are gps before and after, and overlaps before and after
        ...
        if len(frameList) == 5:
            data_list = [all_gps_offset,
                         ...
                         all_insar_res[insar_offset_len_list[0] + insar_offset_len_list[1]:insar_offset_len_list[0] + insar_offset_len_list[1] + insar_offset_len_list[2]],
                         all_insar_res[insar_offset_len_list[0] + insar_offset_len_list[1] + insar_offset_len_list[2] :insar_offset_len_list[0] + insar_offset_len_list[1] + insar_offset_len_list[2] + + insar_offset_len_list[3]]
                         ]
        ...
        if len(frameList) == 2:
            data_list = [all_gps_offset, offset,
        ...
        for i, ax in enumerate(axs):
            ...
            try:
                nanmode = mode(np.around(data[~(data == np.nan)], decimals=1))[0][0]
            except:
                nanmode = 0
...
# fig, ax = plt.subplots()
# poly = frames[frames['track'] == track]
# track_poly = poly.dissolve(by='track', aggfunc='sum')
# poly = tracks[tracks[index] == track]

# track_poly.plot(ax=ax, facecolor='None', edgecolor='grey', alpha=1)  #facecolor='red',
# plt.show()
# fig.savefig(os.path.join(output_dir, track + "_frames.svg"), format='SVG', dpi=300, bbox_inches='tight')
